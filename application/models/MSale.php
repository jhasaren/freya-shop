...
class MSale extends CI_Model {
    ...
    public function validate_select_sale($item,$value,$porcenComision,$type) {
        ...
        if ($query->num_rows() == 0) {
            ...
            log_message("debug", "*******************************************");
            log_message("debug", "Result Comision: ".strval($a));
            log_message("debug", "Comision calculada manual: ".strval($b));
            log_message("debug", "*******************************************");
    ...
    public function liquida_sale() {
        ...
        /*
        * Ajuste: 15/09/2020
        * totalDescuento es aplicado a la venta de productos cuando es tienda 3-shop
        */
...
//                    }
    ...
    public function add_porcentaje_desc($porcentaje,$porcentajeServ) {
        ...
        if ($this->config->item('tipo_negocio') == 1 || $this->config->item('tipo_negocio') == 2) {
            ...
            /*Tipo negocio 3-shop, intercambia porcentaje de servicio y porcentaje descuento*/
            $this->db->query("UPDATE
                        venta_maestro SET
                        porcenDescuento = ".$porcentajeServ.",
                        porcenServicio = ".$porcentaje."
                        WHERE
                        idVenta = ".$this->session->userdata('idSale')."
                        ");
        }
