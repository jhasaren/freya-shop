...
class MReport extends CI_Model {
    ...
    public function payment_salesproduct($fechaIni,$fechaFin) {
        ...
        $query = $this->db->query("SELECT
                                ...
                                FROM venta_detalle v
                                JOIN venta_maestro m ON m.idVenta = v.idVenta
                                JOIN forma_de_pago f ON f.idVenta = m.idVenta
                                JOIN tipo_estado_recibo t ON t.idEstadoRecibo = m.idEstadoRecibo
                                JOIN app_usuarios a ON a.idUsuario = m.idUsuarioCliente
                                LEFT JOIN productos p ON p.idProducto = v.idProducto
                                WHERE
                                ...
                                v.valorEmpleado,
                                v.idEmpleado,
                                concat('[',m.nroRecibo,'] ',t.descEstadoRecibo),
                                (m.valorTotalVenta-m.valorLiquida),
                                m.valorLiquida,
                                concat('[',m.idUsuarioCliente,'] ',a.nombre,' ',a.apellido),
                                m.fechaLiquida,
                                m.fechaPideCuenta
    ...
    /**************************************************************************
     * Nombre del Metodo: payment_ingresosdetail
     * Descripcion: Detalle de ingresos por producto
     * Autor: jhonalexander90@gmail.com
    ...
    /**************************************************************************
     * Nombre del Metodo: abonos_consolidado_sedes
     * Descripcion: Recupera consolidado de abonos en un periodo de 
     * tiempo por Sede.
     * Autor: jhonalexander90@gmail.com
     * Fecha Creacion: 16/09/2020, Ultima modificacion: 
     **************************************************************************/
    public function abonos_consolidado_sedes($fechaIni,$fechaFin) {
    ...
    public function estado_pyg($fechaIni,$fechaFin, $sede) {
        ...
        $queryIngresos = $this->db->query("SELECT
                                        f.idTipoPago,
                                        t.descTipoPago,
                                        sum(f.valorPago) as valorLiquida
                                        FROM forma_de_pago f
                                        JOIN tipo_forma_pago t ON t.idTipoPago = f.idTipoPago
                                        JOIN venta_maestro v ON v.idVenta = f.idVenta
        ...
        if ($queryIngresos->num_rows() == 0) {
            ...
            $data['productos'] = $queryIngresos->result_array();
            $data['adicional'] = null;
    ...
    /**************************************************************************
     * Nombre del Metodo: gastos_consolidado_cat
     * Descripcion: Recupera consolidado de los Gastos en un periodo de 
     * tiempo por Categoria
     * Autor: jhonalexander90@gmail.com
     * Fecha Creacion: 16/09/2020, Ultima modificacion: 
     **************************************************************************/
    public function gastos_consolidado_cat($fechaIni,$fechaFin) {
        
        $query = $this->db->query("SELECT
                                    cg.descCategoria as descTipoGasto,
                                    sum(g.valorGasto) as valor
                                    FROM gasto_maestro g
                                    JOIN categoria_gasto cg ON cg.idCategoriaGasto = g.idCategoriaGasto
                                    WHERE
    ...
    public function payment_fechadia($fechaIni,$fechaFin) {
        ...
        $queryFecha = $this->db->query("SELECT
                                    f.fechaPago as fechaLiquida,
                                    DATE_FORMAT(f.fechaPago,'%d-%b-%y') as fecha,
                                    sum(f.valorPago) as sumPago
                                    FROM forma_de_pago f
                                    JOIN venta_maestro v ON v.idVenta = f.idVenta
    ...
    /**************************************************************************
     * Nombre del Metodo: birthday_ref
     * Descripcion: Recupera los referidos que cumplen años en un mes determinado
     * Autor: jhonalexander90@gmail.com
     * Fecha Creacion: 13/03/2021, Ultima modificacion: 
     **************************************************************************/
    public function birthday_ref($mes) {
        ...
        $query = $this->db->query("SELECT
                                ...
                                FROM
                                referidos_usuario r
                                JOIN app_usuarios a ON a.idUsuario = r.idUsuario
                                WHERE
                                r.activo = 'S'
                                AND r.mesCumple = ".$mes);
    ...
    /**************************************************************************
     * Nombre del Metodo: top_sales_cat
     * Descripcion: Recupera el top 10 de ventas por categoria de articulos
     * Autor: jhonalexander90@gmail.com
     * Fecha Creacion: 16/09/2020, Ultima modificacion: 
     **************************************************************************/
    public function top_sales_cat($fechaIni,$fechaFin) {
        
        $query = $this->db->query("SELECT
                                gs.descGrupoServicio,
                                sum(v.cantidad) as cantidad
                                FROM
                                venta_detalle v
                                JOIN productos p ON p.idProducto = v.idProducto
                                JOIN grupo_servicio gs ON gs.idGrupoServicio = p.idGrupoServicio
                                JOIN venta_maestro vm ON vm.idVenta = v.idVenta
                                WHERE
                                v.idProducto is not null
                                AND vm.fechaPideCuenta BETWEEN '".$fechaIni." 00:00:00' AND '".$fechaFin." 23:59:59'
                                AND vm.idEstadoRecibo = 5
                                GROUP BY gs.descGrupoServicio
                                ORDER BY 2 DESC
                                LIMIT 10");
    ...
    public function tendencia_diasemana($fechaIni,$fechaFin) {
        ...
        $query = $this->db->query("SELECT
                                ...
                                venta_maestro v
                                WHERE
                                v.fechaPideCuenta BETWEEN '".$fechaIni." 00:00:00' AND '".$fechaFin." 23:59:59'
    ...
    public function tendencia_venta_cliente($fechaIni,$fechaFin) {
        ...
        if ($query->num_rows() == 0) {
            ...
            foreach ($data as $rowData){
                ...
                /*ventas de un solo producto*/
                if ($rowData['cantProductos'] == 1){
                    $est1 = $est1 + 1;
                }
                
                /*ventas de dos productos*/
                if ($rowData['cantProductos'] == 2){
                    $est2 = $est2 + 1;
                }
                
                /*ventas con mas de tres productos*/
                if ($rowData['cantProductos'] >= 3){
                    $est3 = $est3 + 1;
                }
                
                /*ventas con cargo adicional*/
                if ($rowData['cantProductos'] > 0 && $rowData['cantAdicional'] > 0){
                    $est4 = $est4 + 1;
                }
