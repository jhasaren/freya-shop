...
function escposticket ($detalleRecibo,$sede,$dirSede,$telsede,$printer,$turno,$nitRecibo){
    ...
    try {
        ...
        $printer -> text(new item('Descuento('.(number_format($detalleRecibo['general']->porcenDescuento*100,2,',','.')).'%)','-'.number_format(($detalleRecibo['general']->valorTotalVenta-$detalleRecibo['general']->valorLiquida),0,',','.')));
