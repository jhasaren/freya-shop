        ...
        if ($this->session->userdata('validated')) {
            ...
            if ($this->MRecurso->validaRecurso(2)){
                ...
                if ($this->config->item('ws_cat_app') == 1){ /*si funcionalidad habilitada*/
                    /*Consulta Modelo obtiene lista de categorias en el Catalogo APP*/
                    $listTypeCat = $this->MProduct->list_cat_app();
                    $info['list_cat_app'] = $listTypeCat;
                }
        ...
        if ($this->session->userdata('validated')) {
            ...
            if (!$this->input->post()){
                ...
                if ($this->MRecurso->validaRecurso(2)){
                    ...
                    $incatalogo = $this->input->post('incatalogo');
                    ...
                    /*if ($this->jasr->validaTipoString($name,1)){*/
                        ...
                        if ($this->jasr->validaTipoString($valor,2) && $this->jasr->validaTipoString($stock,2) && $this->jasr->validaTipoString($unidosis,2) && $this->jasr->validaTipoString($costo,2)){
                            ...
                            if ($this->jasr->validaTipoString($distributionproduct,3)){
                                ...
                                $registerData = $this->MProduct->create_product($name,$valor,$porcent_empleado,$stock,$unidosis,$typeproduct,$costo,$undmedida,$groupservice,$barcode);

                                if ($registerData == TRUE){
                                    
                                    /***************************/
                                    /*Creacion en Catalogo - App*/
                                    if ($this->config->item('ws_cat_app') == 1){
                                        
                                        if ($incatalogo == 1){
                                            
                                            $part = explode(" ", $name);
                                            $refUnica = $part[0];
                                            $this->MProduct->create_product_app($refUnica,$name);
                                        }
                    ...
                    /*if ($this->jasr->validaTipoString($name,1)){*/
                        ...
                        if ($this->jasr->validaTipoString($valor,2) && $this->jasr->validaTipoString($stock,2) && $this->jasr->validaTipoString($unidosis,2) && $this->jasr->validaTipoString($costo,2)){
                            ...
                            if ($this->jasr->validaTipoString($distributionproduct,3)){
                                ...
                                $updateData = $this->MProduct->update_product($idproduct,$name,$valor,$porcent_empleado,$stock,$unidosis,$valueState,$costo,$valueInvent,$barcode);
