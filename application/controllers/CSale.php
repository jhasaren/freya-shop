...
class CSale extends CI_Controller {
        ...
        if ($this->session->userdata('validated')) {
            ...
            if ($this->MRecurso->validaRecurso(9)){
                ...
                if ($this->session->userdata('idSale') != NULL){
                    ...
                    /*
                    * Ajuste: 15/09/2020
                    * desc: se toma descuento de la venta de productos si es tienda 3-shop
                    */
                    if ($this->config->item('tipo_negocio') == 3){
                        $totalServicios = $liquidacion[0]->totalServicios;
                        //$porcenDescuento = $liquidacion[0]->porcenDescuento;
                        $totalDescuento = $liquidacion[1]->totalDescuento;
                    ...
                    if ($totalServicios <= 0 && $totalProductos <= 0 && $totalAdicional <= 0){
                        ...
                        if (($this->session->userdata('sclient') !== NULL) && ($this->session->userdata('sempleado') !== NULL)){
                            ...
                            if (($this->session->userdata('sservicio') !== NULL)){
                                ...
                                /*
                                 * Ajuste: 15/09/2020
                                 * desc: se toma descuento para venta de productos si es tienda 3-shop
                                 */
                                if ($this->config->item('tipo_negocio') == 3){
                                    $valorPagoServicios = $totalServicios-$totalDescuento;
                                    $valorTotalVenta = ($totalServicios+$totalProductos+$totalAdicional);
                                    $valorLiquidaVenta = ($totalProductos+$totalAdicional)-$totalDescuento;
                                } else {
                                    $valorPagoServicios = $totalServicios-$totalDescuento;
                                    $valorTotalVenta = $totalServicios+$totalProductos+$totalAdicional;
                        ...
                        if ($mixpayment != 'on'){
                            ...
                            if (((int)$pagavalor+(int)$valuePayFormas) < (int)$totalPago){
                                ...
                                if ($registerPay == TRUE){
                                    ...
                                    $timestamp = strtotime($detailRecibo['general']->fechaLiquida);
                                    $reciboPDF = $this->detallerecibopdf($this->session->userdata('idSale'),$recibo,$detailRecibo,$timestamp);
                            ...
                            if (($pagavalor+$valuePayFormas) < $totalPago){ /*si el pago parcial es menor que la totalidad de lo que debe pagar*/
                                ...
                                if ($registerPay){
                                    ...
                                    /*Crea PDF del Recibo Parcial / imprime ticket*/
                                    $timestamp = strtotime($detailRecibo['general']->fechaLiquida);
                                    $reciboPDF = $this->pdetallerecibopdf($this->session->userdata('idSale'),$recibo,$detailRecibo,$timestamp);
     ...
     **************************************************************************/
    public function detallerecibopdf($idVenta,$recibo,$detailRecibo,$timestamp) {
    ...
    /**************************************************************************
     * Nombre del Metodo: pdetallerecibopdf
     * Descripcion: Genera recibo de pago en PDF (parcial)
     * Autor: jhonalexander90@gmail.com
     * Fecha Creacion: 08/03/2021, Ultima modificacion: 
     **************************************************************************/
    public function pdetallerecibopdf($idVenta,$recibo,$detailRecibo,$timestamp) {
        ...
        if ($this->session->userdata('validated')) {
            ...
            if ($this->MRecurso->validaRecurso(9)){
                ...
                /*Carga la vista para Imprimir en PDF*/
                $html = $this->load->view('sale/receipt_detail_pdf_p',$reciboDetalle, true);
                $pdf->SetDisplayMode('fullpage');
