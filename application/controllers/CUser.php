...
class CUser extends CI_Controller {
        ...
        if ($this->session->userdata('validated')) {
            ...
            if (!$this->input->post()){
                ...
                if ($this->MRecurso->validaRecurso(7)){
                    ...
                    /*Captura Variables*/
                    $nameRef = strtoupper($this->input->post('nameref'));
                    $diacumple = $this->input->post('diacumpleref');
                    $mescumple = $this->input->post('mescumpleref');
                    $anocumple = $this->input->post('anonaceref');
                    $parentesco = $this->input->post('parentescoref');
                    $observRef = $this->input->post('observRef'); 
                    $cliente = $this->input->post('clientref'); 

                    if ($this->jasr->validaTipoString($nameRef,1)){

                        if ($this->jasr->validaTipoString($diacumple,7)){

                            /*Envia datos al modelo para el registro del referido*/
                            $registerData = $this->MUser->create_referido($nameRef,$diacumple,$mescumple,$anocumple,$parentesco,$cliente,$observRef);
                            if ($registerData == TRUE){

                                $info['message'] = 'Referido registrado Exitosamente';
                                $info['alert'] = 1;
                                $this->module($info);

                            } else {

                                $info['message'] = 'No fue posible registrar el Referido';
                                $info['alert'] = 2;
                                $this->module($info);
                            ...
                            $info['message'] = 'No fue posible agregar el Referido. Dia Cumpleaños incorrecto.';
                            $info['alert'] = 2;
                            $this->module($info);
                        ...
                        $info['message'] = 'No fue posible agregar el Referido. Nombre/Apellido incorrecto.';
                        $info['alert'] = 2;
                        $this->module($info);
    ...
    /**************************************************************************
     * Nombre del Metodo: delreferido
     * Descripcion: Eliminar Referido
     * Autor: jhonalexander90@gmail.com
     * Fecha Creacion: 07/03/2021, Ultima modificacion: 
     **************************************************************************/
    public function delreferido($idReferido){
        
        if ($this->session->userdata('validated')) {
            
            if ($this->MRecurso->validaRecurso(7)){
                
                /*Envia datos al modelo para eliminar el referido*/
                $delData = $this->MUser->delete_referido($idReferido);
                if ($delData == TRUE){

                    $info['message'] = 'Referido Eliminado Exitosamente';
                    $info['alert'] = 1;
                    $this->module($info);

                } else {

                    $info['message'] = 'No fue posible Eliminar el Referido';
                    $info['alert'] = 2;
                    $this->module($info);
