...
class CReport extends CI_Controller {
    ...
    public function module($report) {
        ...
        if ($this->session->userdata('validated')) {
            ...
            if ($this->MRecurso->validaRecurso(10)){
                ...
                if ($report == 'reportBirthdayRef'){

                    /*Consulta Modelo*/
                    $birthdayRef = $this->MReport->birthday_ref(date('m'));
                    
                    $info['birthdayReferidos'] = $birthdayRef;
                    $this->load->view('reports/report_birthday',$info);
    ...
    /**************************************************************************
     * Nombre del Metodo: sendreceipt
     * Descripcion: Envia recibo (cuenta cobro) al cliente
     * Autor: jhonalexander90@gmail.com
        ...
        if ($this->session->userdata('validated')) {
            ...
            if (!$this->input->post()){
                ...
                if ($this->MRecurso->validaRecurso(8)){
                    ...
                    $telsend = $this->input->post('telsend'); 

                    if ($this->jasr->validaTipoString($telsend,10)){
                        
                        /************************************************/
                        /* Enviar SMS */
                        /************************************************/
                        $textSMS = "Carrusel: Compra registrada. Descargar recibo en: https://freya.tiendacarrusel.com/files/recibos/".$receiptcode.".pdf";
                        $numberPhone = $telsend;
                        //$this->MNotify->sendSMS($numberPhone,$textSMS);
                        /************************************************/

                        echo "<br /><span style='font-size: 24px'>SMS Enviado. En unos minutos llegará la notificación.</span><br />Ya puedes cerrar esta ventana";
                        
                    } else {
                        
                        echo "<br /><span style='font-size: 24px'>No es posible enviar SMS. El Número de Teléfono registrado no es válido.</span><br />Ya puedes cerrar esta ventana";
    ...
    public function fidelizationclients() {
        ...
        if ($this->session->userdata('validated')) {
            ...
            if ($this->MRecurso->validaRecurso(10)){
                ...
                $topServices = $this->MReport->top_sales_cat($fechaini,$fechafin);
